<template>
  <tr v-if="dell">
    <td class="check" @click="opened = !opened">
      {{ exam.id }} <i v-if="opened" class="fa fa-circle-plus plus"></i>
      <i v-else class="fa-solid fa-circle-minus minus"></i>
    </td>
    <td>{{ exam.name }}</td>
    <td class="open">{{ exam.description }}</td>
    <td class="open">{{ exam.start_at }}</td>
    <td class="open">{{ exam.end_at }}</td>
    <td class="open">{{ exam.status }}</td>
    <td class="open">{{ exam.type }}</td>
    <td class="open">
      <a
        class="show"
        @click="
    ...
    async Delete() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await axios
            .delete("api_dashboard/exams/" + this.exam.id)
            .then((res) => {
