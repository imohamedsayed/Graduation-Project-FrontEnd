  ...
  setup(props) {
    ...
    onMounted(async () => {
      if (state.user == null || state.user.role_id != 3) {
        ...
        let res = await axios.get("/api_dashboard/exams/" + props.id);
        ...
        if (res.status == 200) {
          ...
          state.type = exam.type == "Normal Exam" ? 0 : 1;
          state.status = exam.status == "unPulished" ? 0 : 1;
        }
      }
    });
    ...
    const EditExam = async () => {
      ...
      if (!v$.value.$error) {
        ...
        try {
          ...
          if (res.status == 200) {
            notification("success", "تم تعديل الامتحان");
          } else {
