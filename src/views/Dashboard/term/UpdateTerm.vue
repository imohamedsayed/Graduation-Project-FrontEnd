      ...
      <div class="sa4d25">
        <div class="container">
          <div class="row">
            ...
            <div class="col-lg-6">
              <div v-if="state.save" class="alert alert-success" role="alert"> تم اضافه ترم دراسى بنجاح . <span style="
                       {
                        font-size: 18px;
                        cursor: pointer;
                              ...
                              <div class="col-lg-6 col-md-12">
                                <div class="ui search focus mt-30 lbel25">
                                  ...
                                  <select v-model="state.year_id" class="">
                                    <option selected disabled value=""> اختيار من القائمة </option>
                                    <option v-for="year in state.years_list" :key="year.id" :value="year.id"> {{
  ...
  setup(props) {
    ...
    const update_semester = async () => {
      ...
      if(!v$.value.$error) {
        ...
        let res = await axios.post("/api_dashboard/semesters/" + props.id,data)
        if(res.status == 200) {
          state.save = true;
        }
      } else {
